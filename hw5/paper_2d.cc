...
paper_2d::paper_2d(const int m_,const int n_,const bool x_prd_,
    ...
    cbase(new color[ml*(n+4)]), cm(cbase+2*ml+2),
 ...
 *                    calculated. */
void paper_2d::initialize(int ntrace_,double dt_pad,double max_spd) {
...
void paper_2d::choose_dt(double dt_pad,double adv_dt,bool verbose) {
...
#pragma omp parallel for
    ...
    // Loop over the primary grid and set the color field
#pragma omp parallel for
    for(int j=0;j<n;j++) {
        double y=ay+dy*(j+0.5),yy=y+0.5;
        color *cp=cm+ml*j;
...
void paper_2d::solve(double duration,int frames) {
    double t0,t1,t2,adt;
...
void paper_2d::step_forward(double dt) {
    int j;
 ...
 * \return The pressure. */
double paper_2d::average_pressure() {
...
inline void paper_2d::vel_eno2(double &ud,double &vd,double hs,field &f0,field &f1,field &f2,field &f3) {
...
inline double paper_2d::eno2(double p0,double p1,double p2,double p3) {
...
void paper_2d::set_boundaries() {
    ...
    // Set left and right ghost values for color field 
    for(color *cp=cm,*ce=cm+n*ml;cp<ce;cp+=ml) {
            cp[-2].prd_bc(cp[m-2]);
            cp[-1].prd_bc(cp[m-1]);
            cp[m].prd_bc(*cp);
            cp[m+1].prd_bc(cp[1]);
    ...
    // Set top and bottom ghost values
    for(color *cp=cm-2,*ce=cm+m+2;cp<ce;cp++) {
            cp[-tl].prd_bc(cp[g-tl]);
            cp[-ml].prd_bc(cp[g-ml]);
            cp[g].prd_bc(*cp);
            cp[g+ml].prd_bc(cp[ml]);
        }
}

/** Sets boundary conditions for the FEM source term computation, taking into
 * account periodicity */
void paper_2d::fem_source_term_conditions() {
...
void paper_2d::update_tracers(double dt) {
    int i,j;
    double x,y;
...
inline void paper_2d::remap_tracer(double &xx,double &yy) {
 ...
 * Gnuplot. */
void paper_2d::output_tracers(const char *prefix,const int sn) {
    if(ntrace==0) return;
...
void paper_2d::write_files(int k) {
 ...
 * \param[in] duration the simulation duration.
 * \param[in] frames the number of frames to save. */
void paper_2d::save_header(double duration, int frames) {
...
void paper_2d::output(const char *prefix,const int mode,const int sn,const bool ghost) {
