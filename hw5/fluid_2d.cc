...
#pragma omp parallel for
    for(j=0;j<n;j++) for(int i=0;i<m;i++) {
        ...
        double ux,vx,uy,vy,&uc=f.u,&vc=f.v,
               rx,gx,bx,ry,gy,by,&rc=f.r,&gc=f.g,&bc=f.b,
...
void fluid_2d::write_files(int k) {
    if(fflags&1) output("r",0,k);
    if(fflags&2) output("g",1,k);
    if(fflags&4) output("b",2,k);
    ...
    *buf=lx;
    ...
    for(j=0;j<ly;j++,fr+=ml) {
        ...
        *buf=ay+(j+disp)*dy;bp=buf+1;
        switch(mode) {
            case 0: while(bp<be) *(bp++)=(fp++)->r;break;
            case 1: while(bp<be) *(bp++)=(fp++)->g;break;
            case 2: while(bp<be) *(bp++)=(fp++)->b;break;
        }
        fwrite(buf,sizeof(float),lx+1,outf);
    }
